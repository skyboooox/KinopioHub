...
尚未发布的 v3 C++20 SDK 使用官方 NATS C 客户端处理 TCP/TLS，提供自动局域网节点和 RAII 清理。当前运行验证覆盖 macOS ARM64；Linux 是待运行验证的源码目标，Windows 尚未实现。
...
应用可通过 `add_subdirectory()` 引入源码并链接 `KinopioHub::kinopiohub`。也可用 `cmake --install build-v3 --prefix /path/to/prefix` 安装，然后使用 `find_package(KinopioHub CONFIG REQUIRED)`。v3 使用新的构建目录。
...
| 操作 | 含义 |
| --- | --- |
| `hub.scope(name).var(name)` | 可复制、指向稳定状态的句柄 |
...
空 optional 表示本地没有可用值；包含 `Json(nullptr)` 的 optional 是 JSON null。`Hub` 不可复制或移动，变量句柄不会让已关闭的 Hub 继续工作。[内存和版本规则](architecture.zh.md)与其他 SDK 相同。
...
默认自动模式可通过托管 NATS Server 连接 WS/WSS **leaf** 上游：将真正的 leaf 入口写入 `mesh.upstreams`。组网使用 `mesh.group`，leaf TLS 使用 `mesh.upstreamTls`，字段为 `handshakeFirst`、`caFile`、`certFile`、`keyFile`，见 [Server 指南](server.zh.md)。
...
其他示例为 `kinopio_example_offline` 和 `kinopio_example_sdk_status`，接受 `KINOPIO_EXAMPLE_SERVERS`、`KINOPIO_TOKEN`、`KINOPIO_EXAMPLE_TLS_FIRST=1`、`KINOPIO_MESH=0`、`KINOPIO_LEAF_SERVERS`。测试见[开发说明](development.zh.md)。v3 替换旧 API 和独立 leaf 运行时，尚无 live 通道 API。
