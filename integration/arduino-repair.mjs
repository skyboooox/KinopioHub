...
const sockets = new Set(); let suppress = false, dropped = 0;
const proxy = net.createServer(client => {
  ...
  upstream.on('data', data => {
    ...
    for (;;) {
      ...
      if (suppress && message && fields[1].includes('.updates.')) dropped++;
...
try {
  ...
  const value = js.scope('devices').var('battery');
  ...
  assert.equal(after.reconnects, before.reconnects);
